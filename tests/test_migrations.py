...
def test_the_committed_migrations_are_valid_and_start_with_the_schemas():
    ...
    assert migrations[0].path == "00_bronze/ddl/create_schemas_v001.sql"
...
def test_run_order_is_schemas_then_layer_folders_then_table_then_version():
    assert run_order(
        "02_gold/ddl/create_gold_agg_trips_daily_v001.sql",
        "01_silver/ddl/alter_silver_trips_v010.sql",
        "01_silver/ddl/create_silver_trips_v001.sql",
        "01_silver/ddl/alter_silver_trips_v002.sql",
        "00_bronze/ddl/create_bronze_tpch_orders_v001.sql",
        *[f"01_silver/ddl/alter_silver_trips_v{v:03d}.sql" for v in range(3, 10)],
        "00_bronze/ddl/create_schemas_v001.sql",
    ) == [
        "00_bronze/ddl/create_schemas_v001.sql",
        "00_bronze/ddl/create_bronze_tpch_orders_v001.sql",
        "01_silver/ddl/create_silver_trips_v001.sql",
        *[f"01_silver/ddl/alter_silver_trips_v{v:03d}.sql" for v in range(2, 11)],  # v010 after v009, not after v001
        "02_gold/ddl/create_gold_agg_trips_daily_v001.sql",
    ]
...
def test_a_renamed_table_runs_after_the_table_it_renames_even_if_its_name_sorts_first():
    ...
    assert run_order(
        "00_bronze/ddl/rename_bronze_trips_to_nyctaxi_trips_v001.sql",
        "00_bronze/ddl/create_bronze_trips_v001.sql",
        "00_bronze/ddl/alter_bronze_trips_v002.sql",
        "00_bronze/ddl/alter_bronze_nyctaxi_trips_v002.sql",
    ) == [
        "00_bronze/ddl/create_bronze_trips_v001.sql",
        "00_bronze/ddl/alter_bronze_trips_v002.sql",
        "00_bronze/ddl/rename_bronze_trips_to_nyctaxi_trips_v001.sql",
...
@pytest.mark.parametrize(
    "paths, message",
    [
        (["01_silver/create_silver_trips_v001.sql"], r"must be in src/<NN_layer>/ddl/"),
        (["00_bronze/ddl/V001__create_bronze_trips.sql"], "must be named"),
        (["00_bronze/ddl/update_bronze_trips_v001.sql"], "must be named"),  # unknown verb
        (["00_bronze/ddl/create_silver_trips_v001.sql"], "a silver migration can't live in 00_bronze/"),
        (["00_bronze/ddl/create_bronze_trips_v001.sql", "00_bronze/ddl/alter_bronze_trips_v001.sql"], "v001 must create the table"),
        (["00_bronze/ddl/create_bronze_trips_v001.sql", "00_bronze/ddl/create_bronze_trips_v002.sql"], "create can only be v001"),
        (["00_bronze/ddl/create_bronze_trips_v001.sql", "00_bronze/ddl/alter_bronze_trips_v003.sql"], r"missing versions \['v002'\]"),
        (["00_bronze/ddl/rename_bronze_trips_to_nyctaxi_trips_v001.sql"], "renames bronze_trips, which has no migrations"),
        (["00_bronze/ddl/rename_bronze_trips_v001.sql"], "a rename must be named rename_<layer>_<old>_to_<new>"),
    ],
)
...
def test_only_migrations_not_yet_applied_are_pending():
    migrations = parse_migrations(files("00_bronze/ddl/create_schemas_v001.sql", "01_silver/ddl/create_silver_trips_v001.sql", "01_silver/ddl/alter_silver_trips_v002.sql"))
    applied = {(m.key, m.version): (m.path, m.checksum) for m in migrations[:2]}

    assert [m.path for m in pending_migrations(migrations, applied)] == ["01_silver/ddl/alter_silver_trips_v002.sql"]
...
def test_editing_an_applied_migration_is_refused():
    original = parse_migrations(files("01_silver/ddl/create_silver_trips_v001.sql"))[0]
...
def test_an_applied_migration_whose_file_is_gone_is_refused():
    migrations = parse_migrations(files("01_silver/ddl/create_silver_trips_v001.sql"))
    applied = {("silver_trips", 1): (migrations[0].path, migrations[0].checksum), ("silver_trips", 2): ("01_silver/ddl/alter_silver_trips_v002.sql", "abc")}
...
def test_generated_bronze_migration_keeps_source_columns_and_adds_metadata():
    ...
    assert path == "00_bronze/ddl/create_bronze_tpch_region_v001.sql"
